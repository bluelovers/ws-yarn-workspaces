...
import { normalizeSemVerObjectInput } from './pruned';
 ...
 * ```
 */
export function createSemVerFromMatch(
	m: RegExpExecArray,
...
): ISimpleSemVer
{
	...
	if (m[minorIndex] !== undefined) version += '.' + m[minorIndex];
	if (m[patchIndex] !== undefined) version += '.' + m[patchIndex];
	obj.version = version;
	...
	else
	{
		...
		let semver = '';
		if (m[operatorIndex]) semver += m[operatorIndex];
		semver += m[majorIndex];
		if (m[minorIndex] !== undefined) semver += '.' + m[minorIndex];
		if (m[patchIndex] !== undefined) semver += '.' + m[patchIndex];
		if (m[releaseIndex]) semver += '-' + m[releaseIndex];
		if (m[buildIndex]) semver += '+' + m[buildIndex];
		obj.semver = semver;
	}
	...
	if (m[operatorIndex]?.length)
	{
		obj.operator = m[operatorIndex] as IOperator;
	}
 ...
 */
export function createSimpleSemVerFromMatch(
	m: RegExpExecArray,
	operatorIndex: number,
	majorIndex: number,
	minorIndex: number,
...
{
	const obj = createSemVerFromMatch(
		m,
		operatorIndex,
		majorIndex,
		minorIndex,
	...
	return new SimpleSemVer(normalizeSemVerObjectInput(obj, {
		init: true,
	})) as any;
}
