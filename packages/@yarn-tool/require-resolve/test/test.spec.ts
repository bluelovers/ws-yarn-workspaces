...
describe('requireResolveCore', () =>
{
	describe('基本解析功能', () =>
	{
		test('應該能夠解析存在的模組', () =>
		{
			...
			expect(result).toMatch(/jest.*build.*index\.js$/);
		});

		test('應該能夠解析帶有自訂對應的模組', () =>
		{
			// tsdx 在 defaultMap 中有特殊對應
			// 由於 tsdx 可能未安裝，我們測試自訂 map 功能
			const customMap = {
...
describe('handleOptionsPaths', () =>
{
	...
	test('應該將 SymbolModuleMain 轉換為實際路徑', () =>
	{
		...
		// SymbolModuleMain 可能在某些環境下返回空陣列
...
describe('_unshiftArray', () =>
{
	test('應該將元素插入陣列開頭', () =>
	{
		const arr = [2, 3, 4];
		const result = _unshiftArray(arr, 1);

		expect(result).toEqual([1, 2, 3, 4]);
		expect(arr).toEqual([1, 2, 3, 4]); // 原地修改
	});
	...
	{
		const arr = ['b', 'c'];
		const result = _unshiftArray(arr, 'a');
...
describe('resolvePackage', () =>
{
	...
	{
		const { pkgJsonLocation, pkgRoot } = resolvePackage('jest');
