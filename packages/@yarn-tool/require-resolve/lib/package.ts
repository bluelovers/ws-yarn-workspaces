 ...
 * ```
 */
export function resolvePackageCore(moduleName: string, options?: IOptionsRequireResolve): IPackageCoreInfo
{
	// 建構解析路徑 / Build resolution paths
	const paths = buildResolvePaths(options ?? {});
	const processedPaths = handleOptionsPaths(paths, options?.cwd);
 ...
 * ```
 */
export function resolvePackageRoot(moduleName: string, options?: IOptionsRequireResolve): string
{
 ...
 * ```
 */
export function resolvePackageJsonLocation(moduleName: string, options?: IOptionsRequireResolve): string
{
 ...
 * ```
 */
export function createResolveLocationFn(moduleName: string, options?: IOptionsRequireResolve): (path: string, ...paths: string[]) => string
{
 ...
 * ```
 */
export function readModulePackageJson<P extends IPackageJson = IPackageJson>(moduleName: string, options?: IOptionsRequireResolve): P
{
 ...
 * ```
 */
export function resolvePackage<P extends IPackageJson = IPackageJson>(moduleName: string, options?: IOptionsRequireResolve): IPackageInfo<P>
{
	...
	return {
		...
		pkg: require(pkgJsonLocation) as P,
